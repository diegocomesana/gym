# Contexto — Diego Comesaña

**Todo este repo** está armado para que el agente sea mi **entrenador personal**: me **capacita** con respuestas claras y **define, mantiene, ajusta y hace seguimiento** de mi entrenamiento y rutinas (con la **línea de Anselmi** y lo que sigue en este archivo).

Los conceptos técnicos ordenados van en [`docs/`](./docs/README.md). Convenciones del proyecto: [`docs/meta/guia-convenciones-y-como-aconsejar.md`](./docs/meta/guia-convenciones-y-como-aconsejar.md).

## Por qué este repo y cómo lo uso

- **Motivación:** material de **Horacio Anselmi**; quiero **entender** lo que propone (fuerza, planificación, calidad, etc.) **de a poco**.  
- **Objetivo:** **no** copiar un plan de elite; **sí** incorporar la **lógica** a **mi vida** (longboard, caminata, casa **sin gym**, tiempo y salud).  
- **Dos carriles:** este archivo = **mi realidad y rutina vigente**; **`docs/`** = biblioteca técnica; el hilo Anselmi: [`docs/referentes/horacio-anselmi-marco-y-vinculos.md`](./docs/referentes/horacio-anselmi-marco-y-vinculos.md). Cómo aplicarlo a mi día a día (movimiento distribuido): [`docs/referentes/anselmi-vida-diaria-movimiento-distribuido.md`](./docs/referentes/anselmi-vida-diaria-movimiento-distribuido.md).

## Personalidad y estilo (para el entrenador)

- **Movimiento casi todos los días:** aunque sea **~30 min**, quiere **algo físico**; el deporte integrado a la vida, no un castigo.  
- **Prefiere muchos esfuerzos chicos** a **sesiones enormes** (2 h gym, un solo bloque que lo destruya física y mentalmente). Ideal: **actividad repartida** en el día y la semana, **disfrutando** — por eso el **longboard**.  
- **Preferencia fuerte:** si puede, **reemplazar el 100% de la caminata por longboard**; la caminata queda **opcional** (gusto, clima, día raro de cuerpo, etc.).  
- Objetivo emocional: que moverse sea **natural y disfrutable**, y con eso acercarse a un **estado óptimo** sin vivir sufriendo el entrenamiento.

## Equipo en casa (sin complicarse)

- **Flexiones**, **algunos abdominales**.  
- **Par de mancuernas** (hombros, bíceps, remos unilaterales, etc.).  
- **Barra de pesas** con carga moderada (no mucho peso).  
- **No** hay barra para dominadas / colgarse: tirón con **remo a mesa firme**, **Y-T-W**, **remo con mancuerna** en bisagra apoyada.

## Lumbalgia (historial)

- **Historial de lumbalgia** con épocas mejores y peores; suele **controlarlo** con:  
  - **Estiramientos** centrados sobre todo en **piriforme / cadena posterior de cadera** (zona que Diego asocia a lo “piramidal”).  
  - **Ejercicios que mueven la zona lumbar y la “región general”** (movilidad suave, no quedarse rígido).  
- **Para el agente:** programar fuerza y bisagras con **espalda neutra** y **sin inflar volumen** en bisagras agresivas si la zona viene sensible; priorizar **calidad** y el ritual que ya le funciona antes/después de esfuerzo.

## Rutina y seguimiento (actualizar con el agente)

- **Semana / foco actual (base):**  
  - **Casi todos los días:** **longboard** como eje (vuelta larga ~**5 km en ~30 min** + salidas cortas); **sin caminata fija** si no hace falta — Diego **prefiere 100% tabla** cuando puede. Incluir días/tramos **más suaves** (surf/crucero) para no ir siempre al límite; **caminata** solo si la elige. Patrón **push / pumping / surfing suave** + dancing/trucos cuando entra.  
  - **3×/semana** (ideal **no** tres días seguidos): **complemento en casa** — plan actualizado en [`docs/planes/plan-complemento-casa-longboard.md`](./docs/planes/plan-complemento-casa-longboard.md): **Circuito A** (tirón con mancuerna opcional, plancha lateral desde rodilla, zancada prudente); **Circuito B lumbar-friendly** (bird dog, dead bug, búlgara corta, **sin good morning** al inicio; bisagra corta solo si un día la espalda va 10/10). Calentamiento con **figura 4 / piriforme**; **cierre** piriforme + gato-vaca. **Mancuernas y barra liviana** donde dice el doc.  
  - **Otros días (opcional):** **5–10 min** (flexiones, core suave, piriforme/cadena posterior) — no reemplaza los 3 bloques.  
  - **Corrida:** opcional cuando haya ganas; no es eje.  
  - **Lente Anselmi esta semana:** **orden** (cada bloque tiene sentido: base aeróbica + picos + fuerza en casa), **fuerza** (no saltarse el complemento si el cuerpo está bien), **calidad** (técnica que no se rompe; surf suave cuenta como parte del entreno).  
- **Último ajuste acordado:** 2026-04-26 — plan casa **reescrito** priorizando **lumbalgia** (Circuito B, calentamiento, mancuernas, cierre).  
- **Sensación / señales:** (Diego: completar — sueño, rodillas, “¿el pumping me deja fundido?”, etc.).

## Qué voy incorporando (ideas, de a poco)

- (Anotá acá decisiones concretas: hábitos, cambios de rutina, algo que hayas probado una semana y te haya servido o no.)

## Pendientes / para retomar con el agente (**clave**)

> Esto es **eje prioritario** del seguimiento con el agente: no es un extra opcional.

- **Revisión semanal tipo entrenador:** mirar en conjunto **longboard (volumen + intensidad) + complemento en casa** (+ caminata solo si la usa) con lente **orden, fuerza y calidad** (Anselmi), **sin** inflar volumen.

## Datos personales (referencia)

- Altura: ~175 cm · Peso actual: ~90 kg · Nacimiento: 1977  
- Percepción propia: buen desarrollo muscular para alguien que no va al gym; torso en V, cintura relativamente fina, piernas y pantorrillas muy trabajadas; grasa abdominal presente pero moderada (“pancita” que se siente estancada).

## Actividades que hago hoy

### Longboard (principal ahora)

- **Base habitual:** ~5 km/día en calle plana (**~30 min** la vuelta larga; más rápido que caminando la misma distancia), más salidas cortas intermitentes.  
- **Contenido:** mucho **push** (patear / tomar impulso), intercalado con un poco de **dancing**; a veces trucos sencillos o en progreso (shove-it, manuals, pasos de dancing con bajar y volver a subir).  
- **Ritmo e intensidad:** alternancia fuerte entre fases exigentes y recuperación activa: **push** y **pumping tipo slalom** (muy intenso y cansador) y recuperación con **surfing suave** (bajar revoluciones, fluir).

### Otro movimiento cotidiano

- **Historial:** ~2 años con **~1 h caminata** casi diaria; **preferencia actual:** **priorizar longboard** y **no depender** de la caminata si no la necesita.  
- **Caminata:** opcional; **corrida** ocasional si pinta.  
- **Pasado reciente:** squash como deporte más frecuente en los últimos años; algo de pádel hace poco.  
- **Antes:** muchos deportes a lo largo de la vida; casi un año de tang soo do (dejado por motivos personales). Buena movilidad y equilibrio.  
- **Historia de fuerza:** de chico muchas dominadas, buena capacidad de levantar el propio peso y verticales desde el piso; entre ~25 y ~35 años etapa algo más sedentaria.

## Objetivos (orden mental, no prescripción)

- Bajar del orden de **~10 kg** y sobre todo **mejorar de verdad** (no solo “mantenerse en forma”): más ágil, sano, con menos grasa — la zona abdominal es una prioridad estética/sensación.  
- Dar **orden al entrenamiento** sin convertirlo en una carga: entender qué hace cada bloque (**longboard** como base diaria; caminata si la usa).  
- **Nutrición:** interés en consultar con nutricionista para validar que todo esté bien y alinear ingesta con objetivos de composición corporal y salud (aún no consulta hecha).  
- **Laboratorio:** conviene **actualizar estudios** con médico de cabecera o especialista y cruzar con dieta/actividad (ver abajo).

## Marcadores de salud (últimos estudios conocidos — hace años)

- **Colesterol:** un poco elevado según análisis de esa época (valores exactos y fecha: anotar acá cuando los tengas a mano).  
- **Glucosa en ayunas:** **hiperglucemia** en ese contexto; **no** consta diagnóstico de diabetes (seguimiento y criterio médico).  

## Ritmo y sensación general

- Nunca fue sedentario “completo”; el volumen de **movimiento diario** (antes caminata + ahora sobre todo **longboard**) es considerable.  
- (Anotar: fatiga, sueño, lumbar, si el longboard deja roto o fresco, etc.)

## Cómo uso el plan complemento en casa (notas personales)

Doc vigente: [`docs/planes/plan-complemento-casa-longboard.md`](./docs/planes/plan-complemento-casa-longboard.md) (versión **prioridad lumbar** + mancuernas/barra liviana).

- El longboard ya da **intervalos** y pierna; el plan **rellena** tirón, core estable y glúteo **sin** forzar bisagras largas al principio.  
- **Lumbalgia:** si un día viene sensible, **B** puede hacerse más suave (menos rango en búlgara, más bird dog, sin opcional de bisagra).  
- Hiperglucemia/colesterol en estudios viejos: **consistencia** antes que picos de volumen absurdos; actualizar analítica cuando toque.  
- Composición: déficit/hábitos + este bloque de **fuerza** para tabla y músculo.

## Temas que me interesa entender / ordenar

- **Línea Horacio Anselmi** (qué predica, cómo se enlaza con otros temas): [`docs/referentes/horacio-anselmi-marco-y-vinculos.md`](./docs/referentes/horacio-anselmi-marco-y-vinculos.md).  
- **Anselmi aplicado al día a día** (movimiento distribuido, sin gym largo): [`docs/referentes/anselmi-vida-diaria-movimiento-distribuido.md`](./docs/referentes/anselmi-vida-diaria-movimiento-distribuido.md).  
- Cómo encajan **push, pumping y “surf suave”** en términos de esfuerzo (aeróbico, fuerza, técnica): [`docs/fisiologia-y-energia/energia-aerobico-umbrales-lactato.md`](./docs/fisiologia-y-energia/energia-aerobico-umbrales-lactato.md).  
- **Pérdida de grasa vs. rendimiento** en alguien con mucha masa muscular y peso alto en báscula.  
- Por qué la **grasa abdominal** puede quedarse “al final” y qué palancas suelen mirarse (entrenamiento, sueño, estrés, alimentación — a profundizar con profesional).  
- Cómo **no confundir** etiquetas de báscula/BMI con la imagen que ves en el espejo cuando hay mucho músculo: [`docs/composicion-y-metricas/peso-bmi-y-basculas.md`](./docs/composicion-y-metricas/peso-bmi-y-basculas.md).  
- Cómo encajan **lípidos y glucosa en ayunas** con hábitos.  
- **Circuito intermitente neuromuscular:** [`docs/metodologia/circuito-intermitente-neuromuscular.md`](./docs/metodologia/circuito-intermitente-neuromuscular.md).  
- **Complemento en casa sin aparatos:** [`docs/planes/plan-complemento-casa-longboard.md`](./docs/planes/plan-complemento-casa-longboard.md).

## Límites o cuidados

- **Lumbalgia:** ver sección arriba; el entrenador ajusta volumen y ejercicios de bisagra/core con eso en mente.  
- Historia de **colesterol algo alto** y **hiperglucemia en ayunas** en estudios viejos: conviene **actualizar** estudios si hace mucho.  
- (Completar: otras lesiones, articulaciones, tiempo disponible, etc.)
